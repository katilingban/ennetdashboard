  ...
  if (input$frequency_options == "Daily") {
    z <- y %>%
      ...
      mutate(`Total views per post per day` = Views / nPosts,
             `Total replies per post per day` = Replies / nPosts,
             `New views per post per day` = `New Views` / nPosts,
             `New replies per post per day` = `New Replies` / nPosts) %>%
  ...
  if (input$frequency_options == "Daily") {
    z <- y %>%
      ...
      mutate(`Total views per post per day` = Views / nPosts,
             ...
             `New replies per post per day` = `New Replies` / nPosts) %>%
      ggplot(mapping = aes(x = `Extraction Date`, 
                           y = `New replies per post per day`)) +
      geom_line(colour = unicef_blue, size = 1, alpha = 0.7) +
      scale_x_date(name = "", 
                   date_breaks = "1 month",
                   date_labels = "%b %Y") %>%
      labs(title = "New replies per post per day by theme",
           x = "",
           y = "No. of new replies") +
      facet_wrap(. ~ Theme, ncol = 1, scales = input$scale_options) +
      unicef_theme
