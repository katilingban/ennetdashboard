...
#checkboxGroupInput(inputId = "themes_options",
#                   label = "Themes",
#                   selected = 1:3,
#                   choiceNames = themes,
#                   choiceValues = seq_len(length(themes)))
...
#observeEvent(length(input$themes_options) > 3 | 
#               length(input$themes_options) < 1, {
#  updateCheckboxGroupInput(session = session,
#                           inputId = "themes_options",
#                           label = "Themes",
#                           selected = input$themes_options,
#                           choiceNames = themes,
#                           choiceValues = seq_len(length(themes)))
#})

selectInput(inputId = "year_options",
            label = "Year",
            choices = c(" ", lubridate::year(topics$Posted)))
```
...
output$monthly_topics <- renderPlotly({
  req(input$themes_options, input$year_options)
  
  if (input$year_options == " ") {
    year_options <- lubridate::year(topics$Posted)
  } else {
    year_options <- input$year_options
  }
  
  z <- topics %>%
    filter(Theme %in% input$themes_options,
           lubridate::year(Posted) %in% year_options) %>%
