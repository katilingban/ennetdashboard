  ...
  if (input$frequency_options == "Daily") {
    z <- y %>%
      ...
      mutate(`Total views per post per day` = Views / nPosts,
             `Total replies per view per day` = Replies / Views,
             `New views per post per day` = `New Views` / nPosts,
             `New replies per new view per day` = `New Replies` / `New Views`) %>%
  ...
  if (input$frequency_options == "Daily") {
    z <- y %>%
      ...
      mutate(`Total views per post per day` = Views / nPosts,
             ...
             `New replies per new view per day` = `New Replies` / `New Views`) %>%
      ggplot(mapping = aes(x = `Extraction Date`, 
                           y = `New replies per new view per day`)) +
      geom_line(colour = unicef_blue, size = 1, alpha = 0.7) +
      scale_x_date(name = "", 
                   date_breaks = "1 month",
                   date_labels = "%b %Y") %>%
      labs(title = "New replies per new view per day by theme",
           x = "",
           y = "No. of new replies") +
      facet_wrap(. ~ Theme, ncol = 1, scales = input$scale_options) +
      unicef_theme
