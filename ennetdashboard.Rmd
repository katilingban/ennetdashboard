...
#selectInput(inputId = "metric_options",
#            label = "Metric",
#            selected = "Posts",
...
output$posts <- renderPlotly({
  ...
  ## Filter to specified themes and time period
  x <- topics %>%
  ...
  if (input$frequency_options == "Weekly") {
    y <- x %>%
      mutate(Week = lubridate::isoweek(Posted),
             Year = lubridate::year(Posted)) %>%
      arrange(Year, Week) %>%
      mutate(`Week Year` = paste("Week ", Week, ", ", Year, sep = "")) %>%
      mutate(`Week Year` = factor(`Week Year`, levels = unique(`Week Year`))) %>%
      group_by(Theme) %>%
      count(`Week Year`) %>%
      ggplot(mapping = aes(x = `Week Year`, y = n)) +
      geom_col(fill = unicef_blue, colour = unicef_blue, alpha = 0.7) +
