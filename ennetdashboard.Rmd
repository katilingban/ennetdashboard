...
output$views <- renderPlotly({
  req(input$theme_options)
  
  ## Get time period
  time_period <- input$year_start:input$year_end
  
  ## Tally dailies and then filter to views and specified themes and time period
  x <- dailies %>%
    group_by(Theme, Interaction, Posted, `Extraction Date`) %>%
  ...
  z <- y %>%
    group_by(Theme, Interaction, Posted, `Extraction Date`) %>%
    summarise(`Interactions per post per day` = nInteractions / nPosts, 
              .groups = "drop") %>%
    filter(Theme %in% input$themes_options,
           Interaction == "Views",
           lubridate::year(Posted) %in% time_period) %>%
    ggplot(mapping = aes(x = `Extraction Date`, 
                         y = `Interactions per post per day`)) %>%
    geom_col(fill = unicef_blue, colour = unicef_blue, alpha = 0.7) %>%
    scale_x_date(name = "", 
                 date_breaks = "1 month",
                 date_labels = "%b %Y") %>%
