...
output$posts <- renderPlotly({
  ...
  if (input$frequency_options == "Weekly") {
    y <- x %>%
      mutate(Week = paste(lubridate::isoyear(Posted),
                          lubridate::isoweek(Posted),
                          lubridate::wday(Posted, week_start = 1), sep = "-")) %>%
      mutate(Week = as.Date(Week, format = "%Y-%V-%u")) %>%
      group_by(Theme) %>%
      count(Week) %>%
      ...
      geom_col(fill = unicef_blue, 
               colour = unicef_blue, 
               alpha = 0.7) +
      scale_x_date(date_breaks = "1 year", date_labels = "%V-%Y") +
