...
if(!require(drlib)) remotes::install_github("dgrtwo/drlib")
...
output$posts <- renderPlotly({
  ...
  if (input$frequency_options == "Daily") {
    y <- x %>%
      ggplot(mapping = aes(x = Posted, y = n)) +
  ...
  if (input$frequency_options == "Weekly") {
    y <- x %>%
      mutate(Week = factor(strftime(Posted, format = "%Y-%V-%u"))) %>%
      group_by(Theme, Posted, Week) %>%
  ...
  if (input$frequency_options == "Monthly") {
    y <- x %>%
      mutate(Month = strftime(x = Posted, format = "%b %Y")) %>%
      group_by(Theme, Posted, Month) %>%
      summarise(n = sum(n), .groups = "drop_last") %>%
      ggplot(mapping = aes(x = Posted, y = n)) +
      geom_col(fill = unicef_blue, colour = unicef_blue, alpha = 0.7) +
      scale_x_date(name = "", 
