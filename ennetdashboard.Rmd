  ...
  if (input$frequency_options == "Daily") {
    z <- daily_interactions %>%
      filter(Theme %in% input$themes_options,
             lubridate::year(`Extraction Date`) %in% time_period) %>%
      group_by(Theme, `Extraction Date`) %>%
      mutate(`Total replies per 1000 views per day` = Replies / (Views / 1000),
             `New replies per 1000 views per day` = `New Replies` / (Views / 1000)) %>%
      ggplot(mapping = aes(x = `Extraction Date`, 
                           y = `New replies per 1000 views per day`)) +
      geom_line(colour = unicef_blue, size = 1, alpha = 0.7) +
      scale_x_date(name = "", 
                   #date_breaks = "1 month",
                   date_labels = "%b %Y") %>%
      labs(title = "New replies per 1000 views per day by theme",
           x = "",
           y = "No. of new replies") +
      facet_wrap(. ~ Theme, ncol = 1, scales = input$scale_options) +
      unicef_theme
  ...
  if (input$frequency_options == "Weekly") {
    z <- weekly_interactions %>%
      filter(Theme %in% input$themes_options,
             lubridate::year(`Extraction Week`) %in% time_period) %>%
      group_by(Theme, `Extraction Week`) %>%
      mutate(`Total replies per 1000 views per week` = Replies / (Views / 1000),
             `New replies per 1000 views per week` = `New Replies` / (Views / 1000)) %>%
      ggplot(mapping = aes(x = `Extraction Week`, 
                           y = `New replies per 1000 views per week`)) +
      geom_line(colour = unicef_blue, size = 1, alpha = 0.7) +
      scale_x_date(name = "", 
                   #date_breaks = "1 month",
                   date_labels = "Week %V-%Y") %>%
      labs(title = "New replies per 1000 views per week by theme",
           x = "",
           y = "No. of new replies") +
      facet_wrap(. ~ Theme, ncol = 1, scales = input$scale_options) +
      unicef_theme
  ...
  if (input$frequency_options == "Monthly") {
    z <- monthly_interactions %>%
      filter(Theme %in% input$themes_options,
             lubridate::year(`Extraction Month`) %in% time_period) %>%
      group_by(Theme, `Extraction Month`) %>%
      mutate(`Total replies per 1000 views per month` = Replies / (Views / 1000),
             `New replies per 1000 views per month` = `New Replies` / (Views / 1000)) %>%
      ggplot(mapping = aes(x = `Extraction Month`, 
                           y = `New replies per 1000 views per month`)) +
      geom_line(colour = unicef_blue, size = 1, alpha = 0.7) +
      scale_x_date(name = "", 
                   #date_breaks = "1 month",
                   date_labels = "%b-%Y") %>%
      labs(title = "New replies per 1000 views per month by theme",
           x = "",
           y = "No. of new replies") +
      facet_wrap(. ~ Theme, ncol = 1, scales = input$scale_options) +
      unicef_theme
  ...
  if (input$frequency_options == "Yearly") {
    z <- yearly_interactions %>%
      filter(Theme %in% input$themes_options,
             lubridate::year(`Extraction Year`) %in% time_period) %>%
      group_by(Theme, `Extraction Year`) %>%
      mutate(`Total replies per 1000 views per year` = Replies / (Views / 1000),
             `New replies per 1000 views per year` = `New Replies` / (Views / 1000)) %>%
      ggplot(mapping = aes(x = `Extraction Year`, 
                           y = `New replies per 1000 views per year`)) +
      geom_line(colour = unicef_blue, size = 1, alpha = 0.7) +
      scale_x_date(name = "", 
                   #date_breaks = "1 month",
                   date_labels = "%Y") %>%
      labs(title = "New replies per 1000 views per year by theme",
           x = "",
           y = "No. of new replies") +
      facet_wrap(. ~ Theme, ncol = 1, scales = input$scale_options) +
      unicef_theme
